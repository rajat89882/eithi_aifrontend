        ...
        <>
            {isMobile && !isOpen && (
                <button
                    ...
                    className="fixed top-4 left-4 z-50 p-2 bg-gray-800 text-gray-200 rounded-md shadow-md hover:bg-gray-700"
                >
                    <Menu size={24} />
                </button>
            )}
...
const SidebarItem = ({ icon, text, href, isOpen }) => {
    ...
    return (
        <li className="list-none">
            <Link
                to={href}
                className={`flex items-center space-x-3 px-4 py-2 text-sm hover:bg-gray-700 ${isActive ? 'bg-gray-800 text-white' : 'bg-gray-900 text-gray-400'
                    }`}
            >
